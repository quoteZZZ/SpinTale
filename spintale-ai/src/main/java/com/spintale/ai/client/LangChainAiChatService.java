...
import com.spintale.ai.core.StreamHandler;
...
public class LangChainAiChatService implements AiChatService
{
    ...
    public void streamChat(ChatRequest request, StreamHandler handler)
    {
        ...
        handler.onComplete(chat(request));
    }
    ...
    private List<dev.langchain4j.data.message.ChatMessage> buildMessages(ChatRequest request)
    {
        ...
        {
            for (com.spintale.ai.core.ChatMessage message : request.getMessages())
            {
        ...
        {
            for (com.spintale.ai.core.ChatMessage message : request.getHistory())
            {
                messages.add(convert(message));
            }
        }
    ...
    private dev.langchain4j.data.message.ChatMessage convert(com.spintale.ai.core.ChatMessage message)
    {
