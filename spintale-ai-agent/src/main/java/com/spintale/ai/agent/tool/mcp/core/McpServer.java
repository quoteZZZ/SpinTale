...
public class McpServer {
    ...
    @Autowired
    public McpServer(List<McpResource> resources, List<McpTool> tools, List<McpPrompt> prompts) {
        this();
        if (resources != null) {
            resources.forEach(this::registerResource);
        }
        if (tools != null) {
            tools.forEach(this::registerTool);
        }
        if (prompts != null) {
            prompts.forEach(this::registerPrompt);
        }
    }
    ...
    private McpResource findResource(String uri) {
        ...
        return resources.values().stream()
                .filter(resource -> uri.startsWith(resource.getUri()))
                .max(Comparator.comparingInt(resource -> resource.getUri().length()))
                .orElse(null);
    }
