...
public class RoutingChatService implements AiChatService {
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        ChatRequest finalProcessedRequest = processedRequest;
    ...
    public void streamChat(ChatRequest request, StreamHandler handler) {
        ...
        StreamHandler wrappedHandler = new StreamHandler() {
            ...
            public void onComplete(ChatResponse response) {
                ChatResponse processedResponse = response;
                for (int i = interceptors.size() - 1; i >= 0; i--) {
                    processedResponse = interceptors.get(i).afterChat(finalProcessedRequest, processedResponse);
                }
            ...
            public void onError(Throwable error) {
                for (AiChatInterceptor interceptor : interceptors) {
                    interceptor.onError(finalProcessedRequest, error);
                }
